...
class AddStudentPopup(QDialog):
    ...
    def capture_face(self):
        ...
        st_address = self.stAddress.text().strip()

        if not st_name or not st_roll:
            self.status_label.setText("Please fill Student Name and Roll Number.")
            return

        cap = cv2.VideoCapture(0, cv2.CAP_DSHOW)
        if not cap.isOpened():
            self.status_label.setText("Unable to open camera. Close other apps and try again.")
            return
        ...
        face_found = False
        self.status_label.setText("Scanning for face... (press 'q' to cancel)")
            ...
            for (x, y, w, h) in faces:
                ...
                try:
                    ...
                    if confidence > 78:
                        ...
                        self.status_label.setText(f"Student's Face already exists (id={id_}, {confidence}%).")
                        already = True
                        face_found = True
                        break
                except Exception:
                ...
                if not already:
                    filename = f"{st_name}_{st_roll}.jpg"
                    save_path = os.path.join(face_folder, filename)
                    
                    cv2.imwrite(save_path, roi_gray)
                    self.status_label.setText(f"Face captured and saved: {filename}")
                    face_found = True
                    break
        ...
        if face_found:
            QMessageBox.information(self, "Success", "Face processed successfully.")
            self.accept()
...
class FaceAttendanceSystem(QMainWindow):
    ...
    def initUI(self):
        ...
        cascade_path = 'C:/Users/AVI SHARMA/Documents/Assignment/haarcascade_frontalface_default.xml'
        ...
        try:
            ...
            classifier_file = "C:/Users/AVI SHARMA/Documents/Assignment/classifier.xml"
